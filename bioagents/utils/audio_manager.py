...
class AudioFileManager:
    ...
    def _generate_audio_filename(
        self, original_filename: str, audio_format: str = "mp3"
    ) -> str:
        """
    ...
    def _generate_unique_audio_filename(
        self, original_filename: str, audio_format: str = "mp3"
    ) -> str:
        """
    ...
    def save_audio_file(
        ...
        if handle_duplicates == "rename":
            target_filename = self._generate_unique_audio_filename(
                original_document_name, audio_format
            )
        elif handle_duplicates == "overwrite":
            target_filename = self._generate_audio_filename(
                original_document_name, audio_format
            )
        elif handle_duplicates == "skip":
            base_filename = self._generate_audio_filename(
                original_document_name, audio_format
            )
        ...
        try:
            ...
            file_info = {
                ...
                "was_renamed": target_filename
                != self._generate_audio_filename(original_document_name, audio_format),
            }
...
class AudioFileProcessor:
    ...
    def process_podcast_audio(
        self, temp_audio_path: Union[str, Path], original_document_name: str, **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        try:
            result = self.audio_manager.save_audio_file(
                temp_audio_path, original_document_name, **kwargs
            )
