    ...
    if isinstance(response_data, dict):
        ...
        if response_data.get('status_code') == 401:
            ...
            if isinstance(detail, dict):
                ...
                if isinstance(error_detail, dict) and error_detail.get('status') == 'quota_exceeded':
...
class BaseAPIClient(ABC):
    ...
    async def call_with_retry(self, *args, **kwargs) -> APICallResult:
        ...
        try:
            ...
            logger.error(f"API call failed after {attempt_count} attempts: {last_exception}")
...
class RobustOpenAIClient(BaseAPIClient):
    ...
    def _validate_api_key(self) -> None:
        ...
        if not self.api_key or not self.api_key.startswith("sk-"):
            raise ValidationError("Invalid OpenAI API key format. Must start with 'sk-'")
    ...
    def _validate_tts_input(
        ...
        if voice not in valid_voices:
            raise ValidationError(f"Invalid voice '{voice}'. Must be one of: {', '.join(valid_voices)}")
        ...
        if response_format not in valid_formats:
            raise ValidationError(f"Invalid response_format '{response_format}'. Must be one of: {', '.join(valid_formats)}")
    ...
    async def text_to_speech(
        ...
        **kwargs
        ...
        # Call the retry function
        return await self._text_to_speech_with_retry(text, voice, model, response_format, speed, **kwargs)
...
class RobustElevenLabsClient(BaseAPIClient):
    ...
    def _validate_api_key(self) -> None:
        ...
        if not self.api_key or not self.api_key.startswith("sk_"):
            raise ValidationError("Invalid ElevenLabs API key format. Must start with 'sk_'")
    ...
    def text_to_speech(
        ...
        **kwargs
        ...
        logger.info(f"Estimated credits required: {required_credits} for {len(text)} characters")
    ...
    async def get_voices(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not result.success:
                raise ElevenLabsAPIError(f"ElevenLabs voices call failed: {result.error}")
...
def create_openai_client(api_key: str, config: Optional[RetryConfig] = None) -> RobustOpenAIClient:
    """
...
def create_elevenlabs_client(api_key: str, config: Optional[RetryConfig] = None) -> RobustElevenLabsClient:
    """
