...
from bioagents.docs.workflow import (
    NotebookLMWorkflow,
    FileInputEvent,
    NotebookOutputEvent,
)
...
class DocumentManager:
    ...
    def save_uploaded_file(
        self, uploaded_file, handle_duplicates: str = "rename"
    ) -> Dict[str, Any]:
        """
...
class DocumentsPage:
        ...
        with col1:
            ...
            if uploaded_files:
                ...
                already_processed = all(
                    name in st.session_state.files_being_processed
                ...
                if not already_processed:
                    ...
                    if duplicate_files:
                        st.warning(
                            f"⚠️ {len(duplicate_files)} file(s) already exist: {', '.join([f.name for f in duplicate_files])}"
                        )
                    ...
                    st.info(
                        "✅ These files have already been processed. Please upload new files to process them."
                    )
                    if st.button(
                        "🔄 Reset Upload State",
                        help="Clear processing history to re-upload these files",
                    ):
            ...
            if st.button("Generate In-Depth Conversation", type="secondary"):
                with st.spinner("Generating podcast... This may take several minutes."):
                    try:
                        ...
                        original_document_name = results.get(
                            "original_document_name", "unknown_document"
                        )
                        ...
                        if audio_result.get("error"):
                            st.error(
                                f"Failed to save audio file: {audio_result['message']}"
                            )
                        else:
    ...
    def _handle_file_uploads(
        self, uploaded_files: List, duplicate_handling: str = "rename"
    ) -> None:
        """
        ...
        if st.button("Process Uploaded Files", type="primary"):
            ...
            for i, uploaded_file in enumerate(uploaded_files):
                ...
                result = self.document_manager.save_uploaded_file(
                    uploaded_file, duplicate_handling
                )
                ...
                if result.get("error", False):
                    ...
                    if result.get("was_renamed", False):
                        st.success(
                            f"✅ Uploaded '{uploaded_file.name}' as '{result['safe_name']}'"
                        )
                    else:
    ...
    def _render_file_management(self) -> None:
        ...
        if not uploaded_files:
            st.info(
                "No files uploaded yet. Use the upload section above to add documents."
            )
            return
    ...
    def _render_file_item(self, file_info: Dict[str, Any], index: int) -> None:
        ...
        with st.container():
            ...
            with col2:
                if st.button(
                    "📄 View", key=f"view_{index}_{hash(file_info.get('path', ''))}"
                ):
            ...
            with col3:
                if st.button(
                    "💬 Analyze",
                    key=f"analyze_{index}_{hash(file_info.get('path', ''))}",
                ):
                    self._analyze_file(file_info)

            with col4:
                if st.button(
                    "🗑️ Delete", key=f"delete_{index}_{hash(file_info.get('path', ''))}"
                ):
    ...
    def _show_file_preview(self, file_info: Dict[str, Any]) -> None:
        ...
        st.info(
            f"File preview for {file_info['name']} will be implemented in future versions."
        )

    def _analyze_file(self, file_info: Dict[str, Any]) -> None:
        """Analyze the file with the BioReasoning Agent."""
        st.info(
            f"File analysis for {file_info['name']} will be integrated with the chat system."
        )
        st.markdown(
            "For now, you can mention this file in the Chat page to discuss its contents."
        )
