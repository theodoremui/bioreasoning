...
class ConcreteReasoningAgent(ReasoningAgent):
    """Concrete implementation of ReasoningAgent for testing."""
...
class TestReasoningAgent:
    """Test ReasoningAgent abstract base class."""
    
    def test_reasoning_agent_creation_default(self):
        """Test creating ReasoningAgent with default parameters."""
        agent = ConcreteReasoningAgent(name="TestAgent")
    ...
    def test_reasoning_agent_creation_custom(self):
        """Test creating ReasoningAgent with custom parameters."""
        custom_instructions = "You are a custom agent."
        agent = ConcreteReasoningAgent(
            name="CustomAgent",
    ...
    def test_construct_response_with_citations(self):
        ...
        response = agent._construct_response(mock_run_result, "judge", AgentRoute.WEB)
    ...
    @patch('bioagents.agents.base_agent.Runner')
    @patch('bioagents.agents.base_agent.gen_trace_id')
    @patch('bioagents.agents.base_agent.trace')
    @pytest.mark.asyncio
    async def test_achat_success(self, mock_trace, mock_gen_trace_id, mock_runner):
    ...
    @patch('bioagents.agents.base_agent.Runner')
    @pytest.mark.asyncio
    async def test_achat_timeout_error(self, mock_runner):
    ...
    @pytest.mark.asyncio
    async def test_achat_general_error(self, mock_logger, mock_runner):
        """Test achat handles general errors."""
        # Setup mock to raise general error
        test_error = Exception("Test error")
        mock_runner.run = AsyncMock(side_effect=test_error)
