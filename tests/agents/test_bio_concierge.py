...
class TestBioConciergeAgent:
    ...
    def test_bio_concierge_creation_default(self):
        ...
        assert len(agent._agent_name_mapping) == 5
    ...
    def test_agent_name_mapping(self):
        """Test that agent name mapping is correctly configured."""
        agent = BioConciergeAgent(name="TestConcierge")
        ...
        expected_mappings = {
            ...
            "BiomedicalAssistant": ("Bio MCP Agent", AgentRoute.BIO_MCP),
            "Bio Concierge": ("Bio Concierge Agent", AgentRoute.UNKNOWN),
        }
        
        assert agent._agent_name_mapping == expected_mappings
    ...
    @pytest.mark.asyncio
    async def test_achat_bio_concierge_response(self):
        agent = BioConciergeAgent(name="TestConcierge")
    ...
    @pytest.mark.asyncio
    async def test_achat_chit_chat_agent_response(self):
        agent = BioConciergeAgent(name="TestConcierge")
    ...
    @pytest.mark.asyncio
    async def test_achat_web_agent_response(self):
        agent = BioConciergeAgent(name="TestConcierge")
    ...
    @pytest.mark.asyncio
    async def test_achat_bio_mcp_agent_response(self):
        agent = BioConciergeAgent(name="TestConcierge")
    ...
    @pytest.mark.asyncio
    async def test_achat_unknown_agent_response(self):
        agent = BioConciergeAgent(name="TestConcierge")
    ...
    @pytest.mark.asyncio
    async def test_achat_no_source_response(self):
        agent = BioConciergeAgent(name="TestConcierge")
    ...
    @pytest.mark.asyncio
    async def test_achat_already_prefixed_response(self):
        agent = BioConciergeAgent(name="TestConcierge")
    ...
    @patch('agents.Runner')
    @pytest.mark.asyncio
    async def test_achat_timeout_error(self, mock_runner, mock_chitchat, mock_web, mock_biomcp):
        import asyncio
    ...
    def test_construct_response_with_agent_info(self):
        ...
        assert response.route == AgentRoute.WEB
    ...
    def test_response_prefixing_and_mapping(self):
        ...
        response = agent._construct_response_with_agent_info(run_result, "Chit Chat Agent: Hello! How are you today?", agent._agent_name_mapping["Chit Chat Agent"][1])
        assert response.response_str == "Chit Chat Agent: Hello! How are you today?"
        assert response.route == agent._agent_name_mapping["Chit Chat Agent"][1] 
    ...
    def test_response_prefixing_web_agent(self):
        ...
        response = agent._construct_response_with_agent_info(run_result, "Web Reasoning Agent: Web result here", agent._agent_name_mapping["Web Reasoning Agent"][1])
        assert response.response_str == "Web Reasoning Agent: Web result here"
        assert response.route == agent._agent_name_mapping["Web Reasoning Agent"][1]
    ...
    def test_response_prefixing_bio_mcp_agent(self):
        ...
        response = agent._construct_response_with_agent_info(run_result, "Bio MCP Agent: BioMCP result", agent._agent_name_mapping["BiomedicalAssistant"][1])
        assert response.response_str == "Bio MCP Agent: BioMCP result"
        assert response.route == agent._agent_name_mapping["BiomedicalAssistant"][1]

    def test_response_prefixing_unknown_agent(self):
        from bioagents.agents.bio_concierge import BioConciergeAgent, AgentRoute
        agent = BioConciergeAgent(name="TestConcierge")
        class RawItem:
            def __init__(self, source, content=None):
                self.source = source
        ...
        response = agent._construct_response_with_agent_info(run_result, "Bio Concierge Agent: Some response", AgentRoute.UNKNOWN)
        assert response.response_str == "Bio Concierge Agent: Some response"
        assert response.route == AgentRoute.UNKNOWN
    ...
    def test_response_prefixing_already_prefixed(self):
        ...
        response = agent._construct_response_with_agent_info(run_result, "Chit Chat Agent: Already prefixed response", agent._agent_name_mapping["Chit Chat Agent"][1])
