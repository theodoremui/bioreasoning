...
class TestRobustOpenAIClient:
    ...
    async def test_text_to_speech_success(self):
        ...
        with patch.object(
            client,
            "call_with_retry",
            return_value=APICallResult(success=True, data=mock_response),
        ):
    ...
    async def test_text_to_speech_api_error(self):
        ...
        with patch.object(
            ...
            return_value=APICallResult(
                success=False, data=None, error=Exception("API Error")
            ),
        ):
...
class TestAPICallResult:
    ...
    def test_successful_result(self):
        ...
        result = APICallResult(
            success=True, data="test_data", attempt_count=1, total_time=1.5
        )
    ...
    def test_failed_result(self):
        ...
        result = APICallResult(
            success=False, data=None, error=error, attempt_count=3, total_time=5.0
        )
...
async def test_check_api_connection_function():
    ...
    with patch.object(
        mock_client_failing, "chat_completion", side_effect=mock_chat_completion_failing
    ):
