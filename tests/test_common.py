...
class TestAgentRoute:
    """Test AgentRoute enum."""
    
    def test_agent_route_values(self):
        """Test that AgentRoute has expected values."""
        assert AgentRoute.BIO_MCP.value == "biomcp"
        assert AgentRoute.WEB.value == "web"
        assert AgentRoute.CHIT_CHAT.value == "chit_chat"
        assert AgentRoute.UNKNOWN.value == "unknown"
    
    def test_agent_route_enum_members(self):
        """Test that all expected enum members exist."""
        expected_members = {"BIO_MCP", "WEB", "CHIT_CHAT", "UNKNOWN"}
        actual_members = {member.name for member in AgentRoute}
...
class TestAgentResponse:
    ...
    def test_agent_response_with_empty_citations(self):
        ...
        assert response.citations == []
        assert response.route == AgentRoute.CHIT_CHAT
